...
#This is the same as all of them working, and the probability of 1 of them breaking added together.  Since there are 4 ways for 1 to break, and 1 way for all of them to work, the probability is (4).0426875 + .81450625 = .98598
#It is worth noting that I used the function(x) logic for R code that I learned from the following website: https://www.r-bloggers.com/2011/04/using-apply-in-r/
...
#Repliacte is a function that replicates a vector or matrix n_sim times, and rbinom is a function that generates a vector of random numbers from a binomial distribution with n_components trials and probability of success p.
...
prob_est <- mean(functional_simulations)
print(prob_est)
#------------------------------------------------------------------------------------------------------------------------------------------------------
#3b
#An internet search engine looks for a keyword in 12 databases, searching for them in random order.  Only 5 of these databases contain the given keyword.  #We need to empirically find the probability that the keyword will be found in at least 2 of the 5 searched databases.
...
for(i in 1:n_sim){
    ...
    #This is equivalent to the hypothetical problem that we were given as it is the same as saying that we are looking for the probability that the keyword is found in at least 2 of the 5 searched databases.
...
#------------------------------------------------------------------------------------------------------------------------------------------------------
#4b
#Suppose a factory has two machines A and B that make up 60% and 40% of the total production respectively.  Of their output, machine A produces 3% defective items where machine B produces 5% defective items.  Find the probability that the given defective part was produced by machine B.
#This is P(Produced by B|Defective) = P(Produced by B and Defective) / P(Defective)
#I drew a tree diagram to solve the theoretical prob. it ended up being: (.4 * .05)/ (.4 * .05 + .6 * .03) = (.4 * .05)/0.038 = .526
